...
if [ "$ACTION" = "2" ]; then
    ...
    sudo rm -f "$SCRIPT_DIR/usb-keyboard-connected_udev"
    sudo rm -f "$SCRIPT_DIR/usb-keyboard-connected"
    sudo rm -f "$SCRIPT_DIR/usb-keyboard-disconnected_udev"
    sudo rm -f "$SCRIPT_DIR/usb-keyboard-disconnected"
...
# Create the udev scripts directory
SCRIPT_DIR="/usr/local/bin"
sudo mkdir -p "$SCRIPT_DIR"

# Create the connection script (keyboard connected)
cat << EOF | sudo tee "$SCRIPT_DIR/usb-keyboard-connected_udev" > /dev/null
#!/bin/bash
/usr/local/bin/usb-keyboard-connected &
EOF
...
cat << EOF | sudo tee "$SCRIPT_DIR/usb-keyboard-disconnected_udev" > /dev/null
#!/bin/bash
/usr/local/bin/usb-keyboard-disconnected &
EOF
...
# Set permissions
sudo chmod +x "$SCRIPT_DIR/usb-keyboard-connected_udev"
sudo chmod +x "$SCRIPT_DIR/usb-keyboard-connected"
sudo chmod +x "$SCRIPT_DIR/usb-keyboard-disconnected_udev"
sudo chmod +x "$SCRIPT_DIR/usb-keyboard-disconnected"
...
ACTION=="add", ATTRS{idVendor}=="$VENDOR_ID", ATTRS{idProduct}=="$PRODUCT_ID", RUN+="$SCRIPT_DIR/usb-keyboard-connected_udev"
ACTION=="remove", ATTRS{idVendor}=="$VENDOR_ID", ATTRS{idProduct}=="$PRODUCT_ID", RUN+="$SCRIPT_DIR/usb-keyboard-disconnected_udev"
EOF
