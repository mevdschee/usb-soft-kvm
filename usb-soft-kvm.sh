...
ACTION=="add", ATTRS{idVendor}=="$VENDOR_ID", ATTRS{idProduct}=="$PRODUCT_ID", RUN+="$SCRIPT_DIR/usb-keyboard-connected"
ACTION=="remove", ATTRS{idVendor}=="$VENDOR_ID", ATTRS{idProduct}=="$PRODUCT_ID", RUN+="$SCRIPT_DIR/usb-keyboard-disconnected"
EOF
